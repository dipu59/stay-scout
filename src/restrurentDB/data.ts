// The latest trends Cards Details
// The latest trends Cards Details
export const LatestCard = [
  {
    id: 1,
    slug: "bella-italia",
    path: "/latestTrand/image1.jpg",
    title: "Bella Italia",
    description:
      "An immersive dining experience that blends exceptional culinary craftsmanship with warm hospitality, inviting ambiance, and carefully curated seasonal menus sourced from local producers. Guests can expect thoughtfully prepared dishes, attentive service, and unique flavor combinations that celebrate tradition while embracing modern techniques, creating memorable meals for intimate dinners, family gatherings, and special celebrations alike.",
    rating: "5.0",
    ratings: 5,
    review: "312",
    dynamic: {
      coverPath: "dynamic/covers/image1.jpg",
      location: "Singapore, Bishan-Ang Mo Kio Park",
      time: "Singapore, 7j/7, 08:00 - 22:00",
      placePath: [],
      phone: "+847 87 37 29 01",
      shortLocation: "Singapore, Bishan",
      shortTime: "7j/7, 08:00 - 22:00",
      websiteLink: "www.bellaitalia.com",
      reviews: {
        username: "Wei Jie",
        location: "Singapore, Little India",
        stars: "5",
        date: "29/11/2023",
        description:
          "The handmade pasta and traditional recipes deliver an authentic Italian experience that lingers. Every course shows care in technique and sourcing, from starters to desserts. Service is attentive and knowledgeable, the atmosphere warm and welcoming, and the tiramisu stands out as a highlight that completes a truly satisfying dining occasion for all guests.",
      },
    },
  },
  {
    id: 2,
    slug: "little-shucker",
    path: "/latestTrand/image2.jpg",
    title: "Little Shucker",
    description:
      "A refined oyster bar offering daily catches, seasonal seafood, and classic preparations presented with precision and freshness. The menu highlights delicate shellfish, raw and cooked selections, and complementary accoutrements that elevate natural flavors. Ideal for casual gatherings or special occasions, the venue balances relaxed ambiance with high-quality ingredients and knowledgeable staff ensuring a memorable culinary outing.",
    rating: "3.6",
    ratings: 3,
    review: "189",
    dynamic: {
      coverPath: "dynamic/covers/image2.jpg",
      location: "Singapore, Marina Bay",
      time: "Singapore, 7j/7, 11:00 - 23:00",
      placePath: [],
      phone: "+847 87 37 29 02",
      shortLocation: "Singapore, Marina Bay",
      shortTime: "7j/7, 11:00 - 23:00",
      websiteLink: "www.littleshucker.com",
      reviews: {
        username: "Sarah Lee",
        location: "Singapore, Downtown",
        stars: "4",
        date: "15/11/2023",
        description:
          "Fresh oysters with impeccable presentation and attentive service create a delightful dining experience. The variety of shellfish and accompanying sauces are thoughtfully chosen to complement each selection. While prices are somewhat premium, the overall quality and care justify the cost. This spot is perfect for seafood lovers seeking freshness, atmosphere, and a place to celebrate special moments.",
      },
    },
  },
  {
    id: 3,
    slug: "marafuku-ramen",
    path: "/latestTrand/image3.jpg",
    title: "Marafuku Ramen",
    description:
      "A specialty ramen house devoted to rich, layered broths, springy noodles, and meticulously selected toppings that showcase authentic Japanese techniques. The menu emphasizes depth of flavor achieved through long-simmered stocks and precise seasoning, while presentation highlights tradition and comfort. It is a must-visit for ramen aficionados seeking an honest, high-quality bowl that satisfies both palate and soul.",
    rating: "5",
    ratings: 5,
    review: "276",
    dynamic: {
      coverPath: "dynamic/covers/image3.jpg",
      location: "Singapore, Orchard Road",
      time: "Singapore, 7j/7, 10:00 - 21:00",
      placePath: [],
      phone: "+847 87 37 29 03",
      shortLocation: "Singapore, Orchard",
      shortTime: "7j/7, 10:00 - 21:00",
      websiteLink: "www.marafukuramen.com",
      reviews: {
        username: "Hiroshi Tanaka",
        location: "Singapore, Shenton Way",
        stars: "5",
        date: "08/11/2023",
        description:
          "Outstanding ramen where broth depth, noodle texture, and topping quality are consistently excellent. Each bowl is balanced and comforting, reflecting skillful preparation and respect for tradition. The staff maintains a friendly and efficient service rhythm even during busy hours. For anyone who appreciates authentic Japanese ramen, this restaurant delivers a satisfying and memorable culinary experience every visit.",
      },
    },
  },
  {
    id: 4,
    slug: "bottega",
    path: "/latestTrand/image4.jpg",
    title: "Bottega",
    description:
      "A contemporary Italian dining destination marrying seasonal ingredients with inventive culinary techniques and elegant presentation. The refined menu changes to reflect the best produce of the moment, while the interior design enhances the experience for intimate dinners or celebratory meals. Expect thoughtful pairings, curated wine options, and an atmosphere crafted to make each evening feel distinctly special and memorable.",
    rating: "3.5",
    ratings: 3,
    review: "243",
    dynamic: {
      coverPath: "dynamic/covers/image5.jpg",
      location: "Singapore, Sentosa",
      time: "Singapore, 7j/7, 12:00 - 23:00",
      placePath: [],
      phone: "+847 87 37 29 04",
      shortLocation: "Singapore, Sentosa",
      shortTime: "7j/7, 12:00 - 23:00",
      websiteLink: "www.bottega.sg",
      reviews: {
        username: "Maria Rossi",
        location: "Singapore, Jurong",
        stars: "4",
        date: "22/10/2023",
        description:
          "The seasonal menu is inventive and the wine selection impressive, offering pairings that elevate each dish. Presentation is elegant and thoughtful, though service speed can vary on busy nights. Overall, the restaurant provides a refined dining atmosphere suited for special occasions, where culinary creativity and ambiance come together to create a memorable evening for guests seeking contemporary Italian fare.",
      },
    },
  },
  {
    id: 5,
    slug: "arabia-nights",
    path: "/latestTrand/image5.jpg",
    title: "Arabia Nights",
    description:
      "An enchanting Middle Eastern culinary journey featuring vibrant spices, traditional techniques, and generous hospitality. The menu offers a variety of mezze, grilled specialties, and aromatic desserts crafted to share, creating lively, communal dining experiences. The warm ambiance and attentive staff complement bold flavors and richly spiced dishes, making it an ideal destination for festive gatherings and memorable cultural dining experiences.",
    rating: "5",
    ratings: 5,
    review: "198",
    dynamic: {
      coverPath: "dynamic/covers/image6.jpg",
      location: "Singapore, Arab Street",
      time: "Singapore, 7j/7, 11:00 - 00:00",
      placePath: [],
      phone: "+847 87 37 29 05",
      shortLocation: "Singapore, Arab Street",
      shortTime: "7j/7, 11:00 - 00:00",
      websiteLink: "www.arabianights.sg",
      reviews: {
        username: "Ahmed Hassan",
        location: "Singapore, Geylang",
        stars: "5",
        date: "10/11/2023",
        description:
          "A rich and authentic dining experience with robust flavors, generous portions, and warm hospitality that transports guests to the heart of Middle Eastern cuisine. The service is welcoming and the ambiance lively, perfect for gatherings and celebrations. Each bite reveals traditional techniques and fragrant spices, leaving diners with lasting impressions and a desire to return for more.",
      },
    },
  },
  {
    id: 6,
    slug: "lokma",
    path: "/latestTrand/image6.jpg",
    title: "Lokma",
    description:
      "A destination for Turkish and Mediterranean flavors, presenting classic meze, grilled specialties, and refined desserts in an elegant setting. The kitchen emphasizes authenticity and hospitality, combining traditional recipes with carefully sourced ingredients. Ideal for those who appreciate layered flavors and generous sharing plates, the restaurant provides a polished dining environment and attentive service to complete the culinary journey.",
    rating: "5",
    ratings: 5,
    review: "156",
    dynamic: {
      coverPath: "dynamic/covers/image7.jpg",
      location: "Singapore, Tampines",
      time: "Singapore, 7j/7, 10:00 - 22:00",
      placePath: [],
      phone: "+847 87 37 29 06",
      shortLocation: "Singapore, Tampines",
      shortTime: "7j/7, 10:00 - 22:00",
      websiteLink: "www.lokma.sg",
      reviews: {
        username: "Fatima Oz",
        location: "Singapore, Clementi",
        stars: "5",
        date: "05/11/2023",
        description:
          "Exceptional Turkish dishes showcase a wide variety of flavors, from savory meze to delightful desserts, all served with genuine hospitality. The meze selection is thoughtfully curated, and the service consistently attentive. This establishment delivers authentic culinary traditions in a refined atmosphere, making it a standout option for anyone seeking an elevated Mediterranean dining experience with both flair and comfort.",
      },
    },
  },
  {
    id: 7,
    slug: "the-snug",
    path: "/latestTrand/image7.jpg",
    title: "The Snug",
    description:
      "A cozy, well-appointed pub offering comfort food and a curated selection of craft beverages in a relaxed setting. The menu focuses on hearty dishes, seasonal sides, and inventive snacks designed for sharing, accompanied by an approachable drink list. It is an ideal spot for casual evenings with friends, informal gatherings, and laid-back celebrations that emphasize conviviality and comfort.",
    rating: "4.2",
    ratings: 4,
    review: "127",
    dynamic: {
      coverPath: "dynamic/covers/image8.jpg",
      location: "Singapore, Clarke Quay",
      time: "Singapore, 7j/7, 16:00 - 02:00",
      placePath: [],
      phone: "+847 87 37 29 07",
      shortLocation: "Singapore, Clarke Quay",
      shortTime: "7j/7, 16:00 - 02:00",
      websiteLink: "www.thesnug.sg",
      reviews: {
        username: "James Wilson",
        location: "Singapore, CBD",
        stars: "4",
        date: "28/10/2023",
        description:
          "A lively pub with an excellent offering of craft beers and comforting dishes that hit the spot after a long day. The atmosphere is welcoming and energetic, the staff friendly, and the menu approachable. It’s a dependable choice for groups and casual nights out where guests can enjoy flavorful comfort food and thoughtful beverage pairings in a relaxed environment.",
      },
    },
  },
  {
    id: 8,
    slug: "starbelly",
    path: "/latestTrand/image8.jpg",
    title: "Starbelly",
    description:
      "A farm-to-table restaurant prioritizing fresh, local ingredients and creative seasonal menus rooted in sustainability and ethical sourcing. The kitchen crafts imaginative plates that highlight provenance and flavor harmony, supported by an inviting dining room and conscientious service. This focus creates a meaningful culinary experience for diners who value transparency, environmental stewardship, and thoughtfully executed modern cuisine.",
    rating: "5",
    ratings: 5,
    review: "267",
    dynamic: {
      coverPath: "dynamic/covers/image9.jpg",
      location: "Singapore, Bukit Timah",
      time: "Singapore, 7j/7, 11:30 - 22:30",
      placePath: [],
      phone: "+847 87 37 29 08",
      shortLocation: "Singapore, Bukit Timah",
      shortTime: "7j/7, 11:30 - 22:30",
      websiteLink: "www.starbelly.sg",
      reviews: {
        username: "Emma Green",
        location: "Singapore, Holland Village",
        stars: "5",
        date: "12/11/2023",
        description:
          "Thoughtful farm-to-table execution with fresh seasonal ingredients and artful plating that celebrates local producers. Each dish highlights flavor clarity and sustainable practices, offering diners a satisfying experience that aligns with conscious dining values. The culinary team’s creativity and commitment to quality make this restaurant a standout choice for guests seeking both delicious food and ethical sourcing.",
      },
    },
  },
  {
    id: 9,
    slug: "iori",
    path: "/latestTrand/image9.jpg",
    title: "Iori",
    description:
      "A refined fusion restaurant blending traditional Japanese techniques with modern culinary innovation to create elegantly plated, flavor-focused dishes. The kitchen emphasizes balance, precision, and seasonal ingredients, resulting in composed plates that appeal to discerning palates. This destination is well suited for special occasions and diners seeking a nuanced, artful approach to contemporary Japanese-inspired cuisine.",
    rating: "4.7",
    ratings: 4,
    review: "298",
    dynamic: {
      coverPath: "dynamic/covers/image10.jpg",
      location: "Singapore, Raffles Place",
      time: "Singapore, 7j/7, 12:00 - 23:00",
      placePath: [],
      phone: "+847 87 37 29 09",
      shortLocation: "Singapore, Raffles",
      shortTime: "7j/7, 12:00 - 23:00",
      websiteLink: "www.iori.sg",
      reviews: {
        username: "Kenji Yamamoto",
        location: "Singapore, Shinjuku",
        stars: "5",
        date: "20/11/2023",
        description:
          "A sophisticated dining experience that skillfully balances traditional Japanese flavors with contemporary techniques, resulting in beautifully plated dishes and complex taste profiles. Attention to detail in preparation and presentation is evident throughout the menu. Service is polished and knowledgeable, making it an ideal choice for those seeking refined fusion cuisine and an elevated dining occasion with memorable culinary craftsmanship.",
      },
    },
  },
  {
    id: 10,
    slug: "ngalley",
    path: "/latestTrand/image10.jpg",
    title: "Ngalley",
    description:
      "A contemporary African restaurant celebrating bold flavors, diverse regional influences, and artistic plating that showcases culinary heritage. The menu highlights unique spice blends, creative ingredient pairings, and textural variety, offering diners an adventurous and memorable experience. With a focus on both tradition and innovation, the venue presents a distinctive dining journey that honors African culinary roots in a modern setting.",
    rating: "3.4",
    ratings: 3,
    review: "142",
    dynamic: {
      coverPath: "dynamic/covers/image1.jpg",
      location: "Singapore, Tiong Bahru",
      time: "Singapore, 7j/7, 10:00 - 21:00",
      placePath: [],
      phone: "+847 87 37 29 10",
      shortLocation: "Singapore, Tiong Bahru",
      shortTime: "7j/7, 10:00 - 21:00",
      websiteLink: "www.ngalley.sg",
      reviews: {
        username: "Amara Okafor",
        location: "Singapore, Bedok",
        stars: "4",
        date: "17/10/2023",
        description:
          "A vibrant and creative menu that explores the richness of African cuisine with bold flavors and imaginative presentations. The dishes are thoughtfully composed and often surprising in their combinations, offering diners a fresh perspective on traditional ingredients. Knowledgeable staff and a welcoming atmosphere enhance the experience, making it a compelling choice for adventurous food lovers seeking something truly distinctive.",
      },
    },
  },
  {
    id: 11,
    slug: "diagonal",
    path: "/latestTrand/image11.jpg",
    title: "Diagonal",
    description:
      "A minimalist, design-forward restaurant focusing on innovative small plates and craft cocktails that emphasize technique, seasonality, and creativity. The menu invites adventurous diners to explore a sequence of thoughtfully constructed bites paired with imaginative beverages. The intimate setting and artistic presentation create a memorable tasting experience that rewards curiosity and a willingness to discover unexpected flavor pairings.",
    rating: "5",
    ratings: 5,
    review: "185",
    dynamic: {
      coverPath: "dynamic/covers/image2.jpg",
      location: "Singapore, Jiak Kim Street",
      time: "Singapore, 7j/7, 17:00 - 23:00",
      placePath: [],
      phone: "+847 87 37 29 11",
      shortLocation: "Singapore, Jiak Kim",
      shortTime: "7j/7, 17:00 - 23:00",
      websiteLink: "www.diagonal.sg",
      reviews: {
        username: "Lucas Chen",
        location: "Singapore, Bugis",
        stars: "5",
        date: "08/11/2023",
        description:
          "Creative small plates and expertly mixed cocktails combine to create an engaging and intimate dining experience. The minimal aesthetic allows the culinary craftsmanship to shine, and the tasting-style format encourages exploration and conversation. Service is attentive and knowledgeable, enhancing the overall experience and making it an excellent choice for guests seeking experimental gastronomy and memorable, thoughtfully executed flavors.",
      },
    },
  },
  {
    id: 12,
    slug: "kitoko",
    path: "/latestTrand/image12.jpg",
    title: "Kitoko",
    description:
      "A Pan-Asian restaurant offering diverse regional dishes interpreted with contemporary flair and respect for authentic flavors. The menu showcases a wide variety of techniques and ingredients from across Asia, thoughtfully adapted to highlight balance and harmony. With generous portions and a focus on quality, the establishment is ideal for diners seeking to explore a broad range of Asian culinary traditions in one cohesive setting.",
    rating: "4.3",
    ratings: 2,
    review: "164",
    dynamic: {
      coverPath: "dynamic/covers/image3.jpg",
      location: "Singapore, Takashimaya",
      time: "Singapore, 7j/7, 11:00 - 22:00",
      placePath: [],
      phone: "+847 87 37 29 12",
      shortLocation: "Singapore, Takashimaya",
      shortTime: "7j/7, 11:00 - 22:00",
      websiteLink: "www.kitoko.sg",
      reviews: {
        username: "Priya Kapoor",
        location: "Singapore, Orchard",
        stars: "4",
        date: "25/10/2023",
        description:
          "A robust Pan-Asian offering that highlights the diversity of flavors across the region, prepared with care and modern sensibilities. The extensive menu provides ample opportunities to sample different culinary styles, and the quality of ingredients and preparation generally impresses. It’s an excellent venue for groups or exploratory diners who appreciate variety, authenticity, and well-executed dishes.",
      },
    },
  },
];
